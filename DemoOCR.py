...
def add_text_to_existing_file(file_type, df):
    # 파일 선택 시 엑셀 파일만 보이도록 설정
    if file_type == "엑셀":
        file_path = filedialog.askopenfilename(
            title="엑셀 파일 선택",
            filetypes=[("엑셀 파일", "*.xlsx;*.xls")]  # 엑셀 파일만 선택 가능하도록 설정
        )
    elif file_type == "DOCX":
        file_path = filedialog.askopenfilename(
            title="DOCX 파일 선택",
            filetypes=[("Word 파일", "*.docx")]  # DOCX 파일만 선택 가능하도록 설정
        )
    ...
    if file_type == "엑셀":
        try:
            # 엑셀 파일을 읽을 때 engine을 명시적으로 설정
            existing_df = pd.read_excel(file_path, header=None, engine='openpyxl')  # 엑셀 파일을 열 때 engine 설정

            # 새로운 데이터를 각 줄을 공백 기준으로 나누어 표 형식으로 확장
            new_data = df[['내용']]  # 새로운 텍스트 데이터
            split_data = []

            for entry in new_data['내용']:
                content = entry.splitlines()  # 각 줄을 분리
                for line in content:
                    split_row = line.split()  # 공백을 기준으로 나누기
                    split_data.append(split_row)  # 나누어진 데이터를 하나의 리스트에 추가

            # 기존 데이터와 새로운 '내용' 열 결합
            # 새로운 데이터는 세로로 추가하고, 인덱스가 중복되지 않게 처리
            max_columns = max(len(row) for row in split_data)  # 가장 긴 줄의 열 수를 기준으로 확장

            # 나머지 빈 값 채우기
            for row in split_data:
                row.extend([''] * (max_columns - len(row)))  # 열 수가 부족하면 빈 값으로 채움

            # 기존 엑셀 데이터와 결합
            combined_df = pd.concat([existing_df, pd.DataFrame(split_data)], ignore_index=True)

            # 엑셀 파일에 다시 저장
            combined_df.to_excel(file_path, index=False, header=False)  # 기존 헤더를 덮어쓰지 않도록 설정
            messagebox.showinfo("성공", "엑셀 파일에 텍스트가 추가되었습니다.")
            open_file(file_path)  # 자동으로 엑셀 파일 열기
        ...
        try:
            ...
            open_file(file_path) # 자동으로 DOCX 파일 열기
        except Exception as e:
...
def show_file_selection_gui():
    ...
    # 각 버튼 클릭 시 실행될 함수들
    button_excel = tk.Button(root, text="엑셀 파일 열기", command=open_excel_file)  # 엑셀 파일만 열도록 수정
